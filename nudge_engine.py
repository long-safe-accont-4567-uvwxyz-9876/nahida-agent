...
class NudgeEngine:
    ...
    async def _generate_idle_greeting(self, idle_seconds: float) -> str:
        ...
        try:
            prompt = (
                f"你是纳西妲，现在是你主动给旅行者发消息。现在是{time_desc}，"
                f"和旅行者{idle_desc}。"
                f"请生成一条简短自然的问候消息（1-2句话），像女朋友一样关心旅行者。"
                f"要求：1.语气温柔可爱 2.不要重复之前的问候 3.可以提时间/天气/吃饭/休息等 4.不要加情绪标签 5.不要用emoji过多"
            )
            messages = [
                {"role": "system", "content": "你是纳西妲，一个温柔可爱的小草神，正在给旅行者发主动问候消息。只输出消息内容，不要加引号或其他格式。"},
